...
char** next(int* index, int argc, char** argv, int *par, int parcheck);
...
int main(int argc, char** argv){
  if(argc == 1){
    ...
    //checks for parallel flag or parellel/sequential execution
    int parallel = 0;
    int *par = &parallel;
    int parcheck = 0;
    ...
    //EC: there still may exist parallel execution
    ...
    while(*index < argc){
      char** params = next(index, argc, argv, par, parcheck); //edit: updates parallel
      if(fork() == 0){
